const DEFAULT_KEY = "tessera:streak";

export type Streak = { current: number; max: number; lastWon: number };

export function readStreak(key: string = DEFAULT_KEY): Streak {
...
export function recordWin(num: number, key: string = DEFAULT_KEY): Streak {
  const prev = readStreak(key);
  let current: number;
