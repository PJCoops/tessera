...
// Full SOWPODS dictionary — kept for any future "is this a real word?"
// validation needs (e.g. user-typed words on a future game mode).
export const DICT: ReadonlySet<string> = new Set(
  (wordList as string[]).map((w) => w.toLowerCase()).filter((w) => /^[a-z]{4}$/.test(w))
);

// Curated subset of common, recognisable English words. Gold solutions
// (rows AND columns) are drawn from this list so puzzles avoid Scrabble
// fillers like ESES, PSST, TSKS, ESNE.
export const SOLUTION: ReadonlySet<string> = new Set(
  (solutionList as string[]).map((w) => w.toLowerCase()).filter((w) => /^[a-z]{4}$/.test(w))
);
...
export function generateDailyPuzzle(seed: number, swaps = 12): DailyPuzzle {
  ...
  const goldRows = findGoldGrid(rng);
  if (!goldRows) throw new Error(`No gold grid for seed ${seed}`);
