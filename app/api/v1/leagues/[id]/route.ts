// GET /api/v1/leagues/:id?mode&num — standings for one mini-league: today's
// board filtered to members + the all-time days-won tally. Members only.
// Bearer-auth mirror of /api/leagues/[id] for the mobile client.
//
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const rl = await rateLimit(req, "v1-league-standings", 60, "1 m");
  if (!rl.ok) {
  ...
  try {
    ...
    console.error("v1 league standings failed:", e);
