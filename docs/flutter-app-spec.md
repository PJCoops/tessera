...
| A | 🔶 **further along than tracked** — per-tile semantic labels (position/letter/state), the status-line `liveRegion`, reduce-motion honouring, and the colour-blind alternate palette (toggle + theme + tokens) were all already built in earlier phases. Closed two real gaps 2026-09-13: the colour-blind preference is now mirrored to `profiles` (`POST /api/v1/settings`, pulled on first sign-in) — it used to be device-local only, silently lost on a second device, despite the DB column existing since Phase B; and a touch-target/text-scale audit found and fixed the mode-segment toggles on leaderboard/history/settings (~24pt tall, now ≥44pt via a `minHeight` constraint), the streak chip (fixed 32pt height, both under the minimum and liable to clip its text at 200% scale — now grows from a `minHeight` floor), and the privacy-policy inline link in the email sign-up sheet (unpadded, opaque hit-testing added). No `TextScaler` clamp exists anywhere in the app, which is correct (WCAG wants ≥200% support, not a clamp); board tiles are intentionally fixed-size and out of scope. Not yet audited: the light/dark × default/CVD contact-sheet QA screen, and Switch Control / Full Keyboard Access focus order. | 1–2 d |
