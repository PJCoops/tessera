...
| 5 | ✅ **done** — global/country daily leaderboard, leagues (list/create/join by manual code + deep link), standings + all-time days-won tally, "verified" affordance (static, every row the server returns is already verified), report-score, invite via native share sheet. New `v1` read routes (`leaderboard`, `leagues`, `leagues/[id]`) + `POST /api/v1/leaderboard/report` + `score_reports` table. Also fixed two v1 routes (`leagues/join`, `account/export`) never authenticating a mobile bearer token. Deep-link handling (`/?join=<code>` and `/join/<code>`) is code-complete but needs Associated Domains (iOS) / verified App Links (Android) — both pending dashboard config (`mobile/README.md` "Deep links"); a shared-result link (`/s/*`) isn't recreated on mobile yet. | 2–3 d |
| 6 | ✅ **done** (app-side) — `en.json`/`es.json` ported with the `t()` helper, used throughout every screen. Still open: es store/legal/push copy commissioned (PJ, not code). | 1–2 d |
| 7 | 🔶 **ads done, IAP not started** — real AdMob integration is live: `google_mobile_ads` + `app_tracking_transparency` wired, an interstitial (post-result, gated on the existing grace/frequency-cap logic) and a banner (History screen), both gated on a full consent flow (`mobile/lib/src/ads/consent_flow.dart`) that runs once after the first solve — a one-line primer before Google UMP (EEA/UK/CH only) and before iOS ATT, matching §8.3's sequencing exactly. Dev flavor always uses Google's public test ad units, never real inventory. Real App IDs/ad units registered in the AdMob console 2026-09-15. The account-level half of the ads-removed entitlement (`profiles.ads_removed` → `adsRemoved` → `adsRemovedProvider`) was already done. Still needed: `purchases_flutter` (RevenueCat) and the remove-ads paywall UI — blocked on creating the RevenueCat project and App Store/Play IAP products. | 1–2 d |
| 8 | 🔶 **started** — local notification reminder was already done (Phase 3-adjacent). Added `POST`/`DELETE /api/v1/device-tokens` (register/deregister a push token, bearer-auth, upserts on `(user_id, token)`) and the matching mobile `AccountApi.registerDeviceToken`/`deregisterDeviceToken` client methods. Still needed: the actual `firebase_messaging` SDK (token generation, background handlers), per-tz server-side scheduling, the three notification triggers (streak-at-risk, new-day, league results), and the payload allowlist — all blocked on creating a Firebase project and running `flutterfire configure` against it. | 2 d |
| 9 | 🔶 **started** — the server-side activation events (`account_created`, `result_recorded`, `sync_completed`) are done: captured from the v1 results routes via `posthog-node` against the existing web PostHog project (`app/lib/server-analytics.ts`), distinct_id is `profiles.analytics_id`. `puzzle_fetched` deferred (needs a platform tag on `GET /api/v1/puzzle`, which web also calls directly). Still needed: `posthog_flutter` client SDK (blocked behind the not-yet-built consent flow, §8.3 — wiring it before that exists would violate the "no analytics before consent" design) and Sentry (needs a new project/DSN, same class of blocker as Firebase/AdMob). | 1–2 d |
| A | ✅ **done** — per-tile semantic labels (position/letter/state), the status-line `liveRegion`, reduce-motion honouring, and the colour-blind alternate palette (toggle + theme + tokens) were all already built in earlier phases. Closed the remaining gaps 2026-09-13: the colour-blind preference is now mirrored to `profiles` (`POST /api/v1/settings`, pulled on first sign-in) — it used to be device-local only, silently lost on a second device, despite the DB column existing since Phase B. A touch-target/text-scale audit found and fixed the mode-segment toggles on leaderboard/history/settings (~24pt tall, now ≥44pt via a `minHeight` constraint), the streak chip (fixed 32pt height, both under the minimum and liable to clip its text at 200% scale — now grows from a `minHeight` floor), and the privacy-policy inline link in the email sign-up sheet (unpadded, opaque hit-testing added). No `TextScaler` clamp exists anywhere in the app, which is correct (WCAG wants ≥200% support, not a clamp); board tiles are intentionally fixed-size and out of scope. The light/dark × default/CVD contact-sheet was already built and verified as `mobile/test/golden/board_golden_test.dart`, 4 golden images covering every combo, all passing. A focus-order/Switch Control audit found the same three mode-segment toggles were invisible to assistive tech (bare `GestureDetector`, no `Semantics`/`Focus` node at all — not just a bad order, an absent one); fixed by swapping to `Semantics(button: true) + Material/InkWell`, the same reachable pattern already used correctly for list rows and the streak chip elsewhere in the app. Nothing else flagged: tile semantics, `InkWell` row focusability, and traversal order across the game/leaderboard/settings screens were all already correct. |  |
