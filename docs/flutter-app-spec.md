...
| A | 🔶 **further along than tracked** — per-tile semantic labels (position/letter/state), the status-line `liveRegion`, reduce-motion honouring, and the colour-blind alternate palette (toggle + theme + tokens) were all already built in earlier phases. Closed one real gap 2026-09-13: the colour-blind preference is now mirrored to `profiles` (`POST /api/v1/settings`, pulled on first sign-in) — it used to be device-local only, silently lost on a second device, despite the DB column existing since Phase B. Not yet audited: touch-target sizing (≥44pt), text-scale to 200% (no fixed-width buttons), the light/dark × default/CVD contact-sheet QA screen, and Switch Control / Full Keyboard Access focus order. | 2–3 d |
