...
| 7 | 🔶 **started** — the account-level half of the ads-removed entitlement (`profiles.ads_removed` exposed via `GET /api/v1/results` as `adsRemoved`, `adsRemovedProvider` on mobile, refreshed on sign-in sync) and the new-player-grace + once-per-day interstitial gating logic (`mobile/lib/src/ads/interstitial_gate.dart`, pure + unit-tested), both buildable without any external account. Still needed: `google_mobile_ads` (AdMob), UMP, ATT, `purchases_flutter` (RevenueCat) packages and actual ad/paywall UI — all blocked on creating the AdMob app, RevenueCat project, and App Store/Play IAP products. | 3–4 d |
